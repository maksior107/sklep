...
import models.{Loyalty, LoyaltyRepository, User, UserRepository}
 ...
 */
@Singleton
class LoyaltyController @Inject()(loyaltiesRepo: LoyaltyRepository, userRepo: UserRepository, cc: MessagesControllerComponents)(implicit ec: ExecutionContext) extends MessagesAbstractController(cc) {

  val loyaltyForm: Form[CreateLoyaltyForm] = Form {
    mapping(
      "user" -> longNumber,
      "points" -> number,
  ...
  val updateLoyaltyForm: Form[UpdateLoyaltyForm] = Form {
    mapping(
      "id" -> longNumber,
      "user" -> longNumber,
      "points" -> number,
...
case class CreateLoyaltyForm(user: Long, points: Int)

case class UpdateLoyaltyForm(id: Long, user: Long, points: Int)
